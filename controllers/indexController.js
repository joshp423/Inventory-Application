...
async function allStockGet(req, res) {
  ...
  res.render("index", {
    title: "Musical Instrument Inventory Application",
    stock,
    links,
    categories,
  });
...
async function createStockGet(req, res) {
  const categories = await db.getAllCategories();
  res.render("stockPages/createStockForm", {
...
const validateProduct = [
  body("productTitle")
    ...
    .withMessage(`Product title ${lengthErrTitle}`),
  body("productDesc")
    .trim()
    .escape()
    .isLength({ min: 1, max: 500 })
    .withMessage(`Product description: ${lengthErrDesc}`),
  body("productPrice")
    .escape()
    .isNumeric()
    .isInt({ min: 1 })
    .withMessage(numErrPrice),
  body("productQuantity")
    .trim()
    .escape()
    .isNumeric()
    .isInt({ min: 1, max: 2000 })
    .withMessage(numErrQuantity),
  body("productCategory").trim().escape(),
  body("productBrand")
    .trim()
    .escape()
    .isAlpha()
    .withMessage(`Product brand ${isAlphaBrand}`)
    .isLength({ min: 1, max: 25 })
    .withMessage(`Product brand ${lengthErrBrand}`),
];
...
async function editStockGet(req, res) {
  ...
  const categories = await db.getAllCategories();
  res.render("stockPages/stockEditForm", {
    title: "Edit Stock",
    links,
    stock: stock[0],
    categories,
...
async function deleteStockGet(req, res) {
  const productid = req.params.stockid;
  const stock = await db.getSelectedStockId(productid);
  res.render("stockPages/stockDeletePage", {
...
async function deleteCategoryGet(req, res) {
  ...
  res.render("categoryPages/categoryDeletePage", {
    title: "Delete Stock Confirmation",
    links,
    category: category,
  });
...
async function viewCategoriesGet(req, res) {
  ...
  const countMap = {};
  counts.forEach((c) => {
    //group by category and use id as key for value lookup from object - converts array to lookup object
    countMap[c.category] = c.count;
  });
  categories.forEach((category) => {
    //then attach to each category
...
async function editCategoryGet(req, res) {
  ...
  res.render("categoryPages/categoryEditForm", {
    title: "Edit Category",
    links,
    category,
  });
...
async function newCategoryGet(req, res) {
  res.render("categoryPages/categoryNewForm", {
